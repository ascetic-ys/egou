<template>
	<view class="container">
		<view class="content">
			...
			<view class="qr-code-img" v-if="type==3">
				<image :src='qrCodeImage3'></image>
			</view>
			<view class="note-info">
				<text>{{type!=3?'扫一扫上面的二维码图案，绑定注册':'游客临时访问二维码'}}</text>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			createUserRegQrcode(){
				...
				this.qrCodeImage1 = this.createQrCode(data)
			},
			createBUserRegQrcode(){
				let data = {
					width: 600,
					scene: 'phoneNumber='+this.userInfo.phoneNumber,
					page: "pages/partner/partner"  //这里按照需求设置值和参数   
				}
				this.qrCodeImage2 = this.createQrCode(data)
			},
			createTempVisitQrcode(){
				let date = new Date()
				let expiredTime = date.getTime()
				let data = {
					width: 600,
					scene: 'expiredTime='+expiredTime,
					page: "pages/index/index"  //这里按照需求设置值和参数   
				}
				this.qrCodeImage3 = this.createQrCode(data)
			},
			createQrcode(data){
				var that = this;
				...
				uni.request({
					...
					success(res) {
						console.log(res)
						return 'data:image/png;base64,'+uni.arrayBufferToBase64(res.data);  //对数据进行转换操作
					},
					fail(e) {
						console.log(e)
						return ''
