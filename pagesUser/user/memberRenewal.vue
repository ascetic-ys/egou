		...
		methods: {
			...
			initPartnerParams(){
				this.partnerForm={
					...
					parentId:this.userInfo.id,//城市服务商ID（销售员）
				}
			},
			agreeYes(){
				this.initPartnerParams()
				console.log("`/pagesUser/user/memberPay?price=${this.protocol.fee}&parentId=${this.userInfo.id}`")
				this.$api.httpPost('protocolRecord/api/save',{
					...
					if(r.code==0){
						...
						uni.navigateTo({
							url:`/pagesUser/user/memberPay?price=${this.protocol.fee}&parentId=${this.userInfo.id}&protocolRecordId=${r.data.id}&validDate=${this.userInfo.validDate}`
						})
					}else{
