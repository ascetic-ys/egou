<template>
	<view class="content">
		<view class="navbar">
			<view class="nav-item">
				<view class="input">
					<pickerAddress @change="changeAddress">{{addressName}}</pickerAddress>
				</view>
			</view>
		</view>
		...
		<view >
			...
			<mescroll-body :bottom='bottom' :up='upOption' @down="downCallback" @up="upCallback" @init="mescrollInit">
				<view v-for="(item,index) in list" :key="index" class="order-item" @tap="toDetailPage(item)">
					...
					<view class="desc-box">
						区域：
						<text class="desc-text">{{item.province||""}}{{item.city||""}}{{item.district||""}}</text>
					</view>
					<view class="desc-box">
						注册日期：
						<text class="desc-text">{{item.regdate}}</text>
					</view>
				</view>
			</mescroll-body>
		</view>
	...
	export default {
		...
		methods: {
			...
			changeAddress(data){
				...
				this.params.province=data.data[0]
				this.params.city=data.data[1]
				this.params.district=data.data[2]
				this.addressName = data.data.join('');
			...
			async loadData(data={}) {
				...
				return this.$api.httpPost('partner/api/list',{
					pageNum,
					pageSize,
					...this.params
				}).then(r=>{
			...
			toDetailPage(item){
				uni.navigateTo({
					url:`/pagesUser/partner/partnerDetail?id=${item.id}`
				})
			},
		},
	}
