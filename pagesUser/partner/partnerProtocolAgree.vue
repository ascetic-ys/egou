...
<script>
	...
	export default{
		...
		methods: {
			...
			agreeYes(){
				this.initPartnerParams()
				console.log("`/pagesUser/partner/partnerPay?price=${this.protocol.fee}&parentId=${this.partner.id}`")
				this.$api.httpPost('protocolRecord/api/save',{
					...
					if(r.code==0){
						...
						uni.navigateTo({
							url:`/pagesUser/partner/partnerPay?price=${this.protocol.fee}&parentId=${this.partner.id}&protocolRecordId=${r.data.id}`
						})
					}else{
